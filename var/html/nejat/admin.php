...
$rs_all = mysql_query("select count(*) as total_all from users WHERE `user_name` <> 'hamza'") or die(mysql_error());
$rs_active = mysql_query("select count(*) as total_active from users where banned='0' AND `user_name` <> 'hamza'") or die(mysql_error());
$rs_total_pending = mysql_query("select count(*) as tot from users where banned='1' AND `user_name` <> 'hamza'");						   
...
<h3 class="titlehdr">Kullanici Ayarlari ...</h3>
     ...
     <table width="80%" border="0" align="left" cellpadding="10" cellspacing="0" style="background-color: #E4F8FA;padding: 2px 5px;border: 1px solid #CAE4FF;" >
	  ...
	  $cond = "WHERE `user_name` <> 'hamza' order by date desc";
	  }
	  if($get['qoption'] == 'banned') {
	  $cond = "where `banned`='1' AND `user_name` <> 'hamza' order by date desc";
	  }
...
//	  $sql = "select * from users $cond "; 
	  ...
	  $sql = "select * from users where `user_email` = '$_REQUEST[q]' or `user_name`='$_REQUEST[q]' and  `user_name` <> 'hamza' ";
	  }
...
<form name "searchform" action="admin.php" method="post">
       <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="table.hovertable">
          ...
          <tr> 
            ...
            <td> <font size="2"><a href="javascript:void(0);" onclick='$("#edit<?php echo $rrows['id'];?>").show("slow");'>Edit</a> 
              </font> </td>
          </tr>
          <tr> 
            <td colspan="7">
			...
			Email:<input id="user_email<?php echo $rrows['id']; ?>" name="user_email<?php echo $rrows['id']; ?>" type="text" size="20" value="<?php echo $rrows['user_email']; ?>" >
                     Seviye: <input id="user_level<?php echo $rrows['id']; ?>" name="user_level<?php echo $rrows['id']; ?>" type="text" size="5" value="<?php echo $rrows['user_level']; ?>" > 1->user,5->admin
